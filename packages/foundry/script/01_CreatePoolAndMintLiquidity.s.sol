...
contract CreatePoolAndAddLiquidityScript is Script, Constants, Config {
    ...
    function tokenApprovals() public {
        mockUSDCToken.approve(address(hookContract), type(uint256).max);
        etfToken.approve(address(hookContract), type(uint256).max);
        if (!mockETH.isAddressZero()) {
            mockETHToken.approve(address(PERMIT2), type(uint256).max);
            PERMIT2.approve(address(mockETHToken), address(posm), type(uint160).max, type(uint48).max);
        }
        if (!mockWBTC.isAddressZero()) {
            mockWBTCToken.approve(address(PERMIT2), type(uint256).max);
            PERMIT2.approve(address(mockWBTCToken), address(posm), type(uint160).max, type(uint48).max);
        }
    }
}
